#!/usr/bin/env python3
#this belongs in apps/components/Mdl_Editor/mdl_workshop.py - Version: 1
# X-Seti - May 2026 - MDL Workshop
# UI base: COL Workshop (theme-correct). DFF/model logic ported from Model Workshop.
...
from apps.components.Mdl_Editor.depends.col_workshop_structures import setup_col_table_structure, populate_col_table
from apps.components.Mdl_Editor.depends.col_workshop_parser import COLParser
from apps.components.Mdl_Editor.depends.col_workshop_loader import COLFile
from apps.gui.tool_menu_mixin import ToolMenuMixin
# MDL Workshop parser system
from apps.components.Mdl_Editor.depends.col_workshop_classes import (COLModel, COLVersion, COLHeader, COLBounds, COLSphere, COLBox, COLVertex, COLFace)
...
class MDLWorkshop(ToolMenuMixin, QWidget): #vers 4
    """MDL Workshop - Main window"""
    ...
    def _build_menus_into_qmenu(self, parent_menu): #vers 1
        """Populate parent_menu with MDL Workshop actions."""
        # File
    ...
    def _create_new_model(self): #vers 1
        ...
        from apps.components.Mdl_Editor.depends.col_workshop_classes import COLModel, COLHeader, COLVersion, COLBounds
        m = COLModel()
    ...
    def _create_new_surface(self): #vers 1
        ...
        if not self.current_col_file:
            ...
            return
        from apps.components.Mdl_Editor.depends.col_workshop_classes import COLModel, COLHeader, COLBounds, COLVersion
    ...
    def _open_surface_edit_dialog(self): #vers 2
        ...
        try:
            open_col_mesh_editor = None  # col_mesh_editor not available in standalone
    ...
    def _build_col_from_txd(self): #vers 2
        ...
        from apps.components.Mdl_Editor.depends.col_workshop_classes import COLModel, COLVersion, COLBounds
    ...
    def _convert_surface(self): #vers 2
        ...
        from apps.components.Mdl_Editor.depends.col_workshop_classes import COLVersion
        model = self._get_selected_model()
        if not model:
...
#------ Col functions
    ...
    def _find_col_via_db(self, model_name: str) -> bool: #vers 1
        ...
        try:
            ...
            if mw and hasattr(mw, 'log_message'):
                mw.log_message(
                    f"MDL Workshop: loaded {entry_name} "
    ...
    def _open_col_from_img_entry(self, img, entry): #vers 1
        ...
        try:
            ...
            self.setWindowTitle(
                f"MDL Workshop — {entry.name} (from {os.path.basename(img.file_path)})")
            if self.main_window and hasattr(self.main_window, 'log_message'):
                self.main_window.log_message(
                    f"MDL Workshop: opened {entry.name} from {os.path.basename(img.file_path)}")
        except Exception as e:
...
# ----- Render functions
    ...
    def _show_col_info(self): #vers 4
        ...
        dialog = QDialog(self)
        dialog.setWindowTitle("About MDL Workshop")
        dialog.setMinimumWidth(600)
        dialog.setMinimumHeight(500)
        ...
        # Header
        header = QLabel(f"MDL Workshop - {App_name}")
        ...
        info_text = """<b>MDL Workshop Capabilities:</b><br><br>
...
#class SvgIcons: #vers 1 - Once functions are updated this class will be moved to the bottom
    ...
    # ── MDL/DFF Logic (ported from Model Workshop) ──────────────────
...
def open_col_workshop(main_window, img_path=None): #vers 2
    """Open MDL Workshop - embedded in tab if main_window has tab widget, standalone otherwise"""
    try:
        ...
        if not main_window or not hasattr(main_window, 'main_tab_widget'):
            ...
            workshop.setWindowTitle(f"MDL Workshop - {App_name}")
        ...
        tab_label = os.path.splitext(os.path.basename(img_path))[0] if img_path else "MDL Workshop"
        try:
...
def open_model_workshop(main_window, dff_path=None,
    ...
    try:
        ...
        if main_window and hasattr(main_window, 'main_tab_widget'):
            ...
            layout.setContentsMargins(0, 0, 0, 0)
            workshop = MDLWorkshop(main_window=main_window, parent=container)
